...
[Posit Cloud Connect](https://connect.posit.cloud) so that students can access
it.
...
## Deploying the app to [Posit Cloud Connect](https://connect.posit.cloud)

::: {.callout-important}
### Deploying the app in "Production" vs. "Test" mode

When the app is already deployed to [Cloud Connect](https://connect.posit.cloud)
and running, in order to avoid a collision (which would affect the app usage by
the students), a "test account" must be used in
[Cloud Connect](https://connect.posit.cloud). For example, with email address
"johndoe@psi.uned.es", create account "john-does-test" in
[Cloud Connect](https://connect.posit.cloud).
...
1. Log in to [Cloud Connect](https://connect.posit.cloud) using the email
...
2. In Rstudio, with file ["app.R"](app.R) open in the
   editor, click on the right-side down arrow of button "Publish" (upper-right
   corner) and then on "Manage Accounts...".

3. In the "Options" window, click on "Connect...", then on "Posit Connect Cloud"
   and "Connect Account". This will open an authorizatin page in the default web
   browser.
...
5. Return to Rstudio. The "Options" window will now show the
   [Cloud Connect](https://connect.posit.cloud) account under "Publishing
   Accounts". Click on "Ok" to close the "Options" window.

<!-- TODO: Continue here (using script); also, add URL customization -->
